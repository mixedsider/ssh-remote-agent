...
│        ├─ sshfs 마운트 (동일 절대경로, reconnect 옵션)                 │
...
- **실행**: bash만 SSH로 원격 위임
- **동일경로 마운트**: 원격 `/home/user/proj` → 로컬도 `/home/user/proj` 에 마운트 → 경로 매핑 로직 자체를 제거
...
`~/.ssh-agent/remotes.jsonc` (JSONC — 주석 허용, 사람이 직접 편집 가능)
...
`ssh-agent init --remote <key>:<remote_path>` 실행 시:
1. 레지스트리에서 `<key>` 조회
2. sshfs로 `<remote_path>`를 **동일 절대경로**에 마운트
...
{
  "key": "gpu",                    // 레지스트리 key
  "remotePath": "/home/user/proj", // 원격 실제 경로 (= 로컬 마운트 경로)
  "mountRoot": "/home/user/proj"   // 로컬 마운트 지점 (동일)
}
```
...
```bash
# 원격 레지스트리 관리
ssh-agent remote add <user@host> --key <key> [--port N] [--identity PATH] [--base PATH]
ssh-agent remote add --key <key> --ssh-host <alias>     # ~/.ssh/config 재사용
ssh-agent remote list
ssh-agent remote remove <key>

# 프로젝트를 원격 모드로 초기화
ssh-agent init --remote <key>:<remote_path>
ssh-agent init --remote <key>:<remote_path> --no-mount

# 마운트 관리
ssh-agent mount <key>:<remote_path>               # 재마운트
ssh-agent unmount <mountRoot>
ssh-agent status <mountRoot>                      # 마운트/연결 상태 확인
```
...
- **배포**: npm 패키지용 `dist/cli.js` build를 유지하고, release용 단일 실행 파일은 `bun run build:standalone`으로 `dist/ssh-agent` 생성
...
> 참고: 이 샌드박스에서는 `/dev/fuse` 노출 후 일반 SSHFS mount와 standalone build는 검증했지만,
> localhost 동일경로 mount는 SSHFS 자체가 `read: Interrupted system call`로 실패했다. 실제 원격
> 머신의 동일 절대경로 mount는 사용자 환경에서 최종 확인이 필요하다.
