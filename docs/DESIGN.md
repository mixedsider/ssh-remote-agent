...
│   │   (CLI + 파일편집, passwordless, ~/.ssh/config 별칭 재사용)         │
...
프로젝트 remote 설정 (예: `.opencode/ssh-agent.jsonc` 또는 opencode config 확장):
```jsonc
{
  "remote": {
    "key": "gpu",                    // 레지스트리 key
...
```bash
# 원격 레지스트리 관리
ssh-agent remote add <key> <user@host> [--port N] [--identity PATH] [--base PATH]
ssh-agent remote add <key> --ssh-host <alias>     # ~/.ssh/config 재사용
ssh-agent remote list
ssh-agent remote remove <key>

# 프로젝트를 원격 모드로 초기화
ssh-agent init --remote <key>:<remote_path>
ssh-agent init --local                            # 명시적 로컬 (또는 그냥 init 안 함)

# 마운트 관리
ssh-agent mount <project>                         # 재마운트
ssh-agent unmount <project>
ssh-agent status                                  # 마운트/연결 상태 확인
```
...
7. ✅ `remote add/list/remove` (`src/cli.ts`)
...
> 참고: 이 샌드박스에는 `sshfs` 미설치 + localhost SSH 미구성이라 10·11은 사용자 환경에서
> 최종 확인이 필요. 명령 생성·디코드·실행 메커니즘 자체는 로컬 셸로 완전히 검증 완료.
